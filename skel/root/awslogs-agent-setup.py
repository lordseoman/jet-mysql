...
/usr/bin/env -i HTTPS_PROXY=$HTTPS_PROXY HTTP_PROXY=$HTTP_PROXY NO_PROXY=$NO_PROXY AWS_CONFIG_FILE=$AWS_CONFIG_FILE$ HOME=$HOME$ $NICE_PATH$ -n 4 $CLIPATH$ logs push --config-file $AGENT_CONFIG_FILE$ --additional-configs-dir $AGENT_ADDITIONAL_CONFIGS_DIR$
"""
...
class CloudWatchLogsAgentSetup:
    ...
    def install_awslogs_cli(self):
        ...
        # Setup the awscli using a local file if available.
    ...
    def load_local_config(self):
        fname = os.path.join(HOME, 'awscli_creds.py')
        if os.path.exists(fname):
            import awscli_creds
            subprocess.call([AWSCLI_CMD, 'configure', 'set', 'aws_access_key_id', awscli_creds.AWS_ACCESS_KEY_ID], env=DEFAULT_ENV)
            subprocess.call([AWSCLI_CMD, 'configure', 'set', 'aws_secret_access_key', awscli_creds.AWS_SECRET_ACCESS_KEY], env=DEFAULT_ENV)
            subprocess.call([AWSCLI_CMD, 'configure', 'set', 'default.output', awscli_creds.AWS_OUTPUT], env=DEFAULT_ENV)
            subprocess.call([AWSCLI_CMD, 'configure', 'set', 'default.region', awscli_creds.AWS_REGION], env=DEFAULT_ENV)
            self.region = awscli_creds.AWS_REGION
        elif self.region:
            subprocess.call([AWSCLI_CMD, 'configure', 'set', 'default.region', self.region], env=DEFAULT_ENV)
        else:
            fail("--region is required or awscli_creds.py needs AWS_REGION" % err, UNSUPPORTED_REGION)
    ...
    def run(self, cmd, **kwargs):
        p = subprocess.Popen(cmd, shell=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, **kwargs)
